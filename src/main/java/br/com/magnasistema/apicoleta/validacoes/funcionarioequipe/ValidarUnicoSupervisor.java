...
@Component
public class ValidarUnicoSupervisor implements ValidadorFuncionarioEquipe {
	...
	public void validar(FuncionarioEquipeDtoCadastro dados) {
		...
		if (TipoFuncao.SUPERVISOR.equals(funcionario.getFuncao())) {

			var equipe = equipeRepository.getReferenceById(dados.idEquipe());

			for (Funcionario f : equipe.getFuncionarios()) {
				if (TipoFuncao.SUPERVISOR.equals(f.getFuncao())) {
					throw new ValidacaoException("Já existe um supervisor na equipe.");
				}
			}
		}
